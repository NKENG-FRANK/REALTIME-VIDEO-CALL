-- PostgreSQL Schema for User-Management Microservice
...
CREATE TABLE call_logs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    group_title VARCHAR(150), -- Nullable for 1-on-1 calls, set for group calls
...
-- 6. Call Participants Table (Participant call status & missed call tracking)
CREATE TABLE call_participants (
    ...
    is_read BOOLEAN NOT NULL DEFAULT FALSE, -- Missed call unread status
...
-- 8. User Sessions Table (Security & Tokens)
...
CREATE INDEX idx_user_contacts_owner ON user_contacts(owner_user_id);
CREATE INDEX idx_user_contacts_contact ON user_contacts(contact_user_id);
...
CREATE INDEX idx_call_participants_status ON call_participants(participant_status);
